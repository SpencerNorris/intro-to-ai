...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        #Get nearest ghost's estimated location
        closest_loc = None
        closest_dist = float("inf")
        for distribution in livingGhostPositionDistributions:
            loc = max(distribution.iteritems(), key=lambda tup: tup[1])[0] # most certain location
            dist = self.distancer.getDistance(pacmanPosition, loc) # dist to that location
            if dist < closest_dist:
                closest_loc = loc
                closest_dist = dist

        #Get action that will bring us closest
        best_action = None
        shortest_path = float("inf")
        for action in legal:
            successor = Actions.getSuccessor(pacmanPosition, action)
            length = self.distancer.getDistance(successor, closest_loc)
