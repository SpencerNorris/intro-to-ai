...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        #noisyDistance will be None when the ghost is in jail;
        #so if None, we know the ghost has to be in jail
        if noisyDistance is None:
            allPossible[self.getJailPosition()] = 1.0

        #Belief update
        for p in self.legalPositions:
            #The ghost has to be in jail, so set all positions that aren't jail to 0
            if noisyDistance is None and not p == self.getJailPosition():
                allPossible[p] = 0.0

            #B(ND_t) is proportional to P(ND_t | TD_t)*P(TD_t | ND_1:t-1)
            else:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for position in self.legalPositions:
            position_distribution = self.getPositionDistribution(self.setGhostPosition(gameState, position))

            for next_position, next_prob in position_distribution.items():
                updated_beliefs[next_position] += curr_beliefs[position] * next_prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        self.particles = []
        per_pos_count = self.numParticles / len(self.legalPositions)
        for i in range(len(self.legalPositions)):
            for x in range(per_pos_count): self.particles.append(self.legalPositions[i]) 
    ...
    def observe(self, observation, gameState):
        ...
        #noisyDistance is None when ghost in jail
        if noisyDistance is None:
            for x in range(len(self.particles)):
                self.particles[x] = self.getJailPosition()

        else:
            next_distribution = util.Counter()
            updated_particles = []
            for particle in self.particles:
                dist = util.manhattanDistance(pacmanPosition, particle)
                next_distribution[particle] += emissionModel[dist]
            
            if next_distribution.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                particle_update = []
                for x in range(self.numParticles): particle_update.append(util.sample(next_distribution))
    ...
    def elapseTime(self, gameState):
        ...
        particle_update = []
        for particle in self.particles:
            update_distribution = self.getPositionDistribution(self.setGhostPosition(gameState, particle))
            particle_update.append(util.sample(update_distribution))
        self.particles = particle_update
    ...
    def getBeliefDistribution(self):
        ...
        for position in self.legalPositions : beliefs[position] = 0
        for particle in self.particles: beliefs[particle] += 1
        #for position in self.legalPositions:
            #beliefs[position] = float(beliefs[position]) / float(self.numParticles)
        beliefs.normalize()
        return beliefs
